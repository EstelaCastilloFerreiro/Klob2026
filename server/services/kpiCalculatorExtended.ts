...
// CRITICAL: Enrich ventas with product temporada for devoluciones
// Streamlit merges ventas with productos to inherit temporada for refunds with A1900 or empty temporada
// This ensures devoluciones are visible when filtering by season
function enrichVentasWithProductoTemporada(
  ventas: VentasData[],
  productos: ProductosData[]
): VentasData[] {
  // DEBUG: Check how many products have codigoUnico
  const productosConCodigo = productos.filter(p => p.codigoUnico && p.codigoUnico.trim());
  console.log(`🔍 DEBUG Productos: Total ${productos.length}, con codigoUnico: ${productosConCodigo.length}`);
  
  // Sample first 5 productos with codigo
  const sampleProductos = productosConCodigo.slice(0, 5);
  console.log('🔍 Muestra productos (primeros 5):');
  sampleProductos.forEach((p, i) => {
    console.log(`   ${i + 1}. Código: "${p.codigoUnico}" | Tema: "${p.tema}"`);
  });
  
  // Create a map of codigoUnico -> producto for fast lookups
  // Use full codigoUnico without slicing to ensure accurate matching
  const productosMap = new Map<string, ProductosData>();
  productos.forEach(p => {
    if (p.codigoUnico) {
      const codigo = p.codigoUnico.trim().toUpperCase();
      productosMap.set(codigo, p);
    }
  });
  
  console.log(`🔄 Enriqueciendo ventas con temporada de productos (${productosMap.size} productos disponibles)`);
  
  // DEBUG: Sample ventas to be enriched
  const ventasToEnrich = ventas.filter(v => !v.temporada || v.temporada === 'A1900').slice(0, 5);
  console.log('🔍 Muestra ventas a enriquecer (primeras 5):');
  ventasToEnrich.forEach((v, i) => {
    console.log(`   ${i + 1}. Código: "${v.codigoUnico}" | Temporada actual: "${v.temporada}"`);
  });
  ...
  // Enrich ventas with producto temporada if venta temporada is A1900 or empty
  const enrichedVentas = ventas.map(venta => {
    // Only enrich if temporada is A1900 or empty/falsy
    if (!venta.temporada || venta.temporada === 'A1900') {
      matchAttempts++;
      const codigoNormalizado = (venta.codigoUnico || '').trim().toUpperCase();
      const producto = productosMap.get(codigoNormalizado);
      
      // DEBUG: Log first 3 failed matches
      if (!producto && noMatchCount < 3) {
        noMatchCount++;
        console.log(`   ❌ No match ${noMatchCount}: Venta código "${codigoNormalizado}" no encontrado en productos`);
      }
      
      if (producto && producto.tema) {
        // Extract temporada from tema (e.g., "T_PV25 05 MARFIL_P" -> "PV25")
        // FIXED: Correct regex to match PV or OI followed by 2 digits
        const temaMatch = producto.tema.match(/(PV|OI)\d{2}/i);
        if (temaMatch) {
          const temporadaFromTema = temaMatch[0].toUpperCase();
          enrichedCount++;
          
          // Debug first 3 enrichments
          if (enrichedCount <= 3) {
            console.log(`   ✅ Enriquecido ${enrichedCount}: ${venta.codigoUnico} ${venta.temporada} -> ${temporadaFromTema} (tema: ${producto.tema})`);
          }
          
          return {
            ...venta,
            temporada: temporadaFromTema,
            _temporadaEnriched: true, // Debug flag
          };
        }
      }
    }
  ...
  console.log(`✅ Enriquecidas ${enrichedCount} ventas de ${matchAttempts} intentos (${productos.length} productos totales)`);
  
  return enrichedVentas;
}

// Replicate Streamlit's filter_by_season function (app_correct.py lines 97-101)
// This MUST be applied FIRST, before any other filtering
function filterBySeason(ventas: VentasData[], temporada?: string): VentasData[] {
  if (!temporada || temporada === 'Todas las temporadas') {
    return ventas;
  }
  // Exact Streamlit logic: df_ventas[df_ventas["Temporada"] == temporada_seleccionada]
  return ventas.filter(v => v.temporada === temporada);
}
...
export function calculateExtendedDashboardData(
  ...
  console.log('📊 Extended Dashboard - Filters:', JSON.stringify(filters));
  console.log(`📊 Total ventas before filters: ${ventas.length}`);
  
  // STEP 1: Enrich ventas with producto temporada (critical for devoluciones with A1900)
  const enrichedVentas = enrichVentasWithProductoTemporada(ventas, productos);
  
  // STEP 2: Apply season filter FIRST (exactly like Streamlit's filter_by_season)
  // This must happen BEFORE any other filtering to preserve devoluciones
  let filteredVentas = filterBySeason(enrichedVentas, filters?.temporada);
  console.log(`📊 Total ventas after season filter: ${filteredVentas.length}`);
  
  // Then apply other filters (tienda, familia, etc.) but NOT temporada again
  if (filters) {
    const filtersWithoutTemporada = { ...filters };
    delete filtersWithoutTemporada.temporada; // Remove to avoid double-filtering
    filteredVentas = applyFilters(filteredVentas, filtersWithoutTemporada);
  }
  console.log(`📊 Total ventas after all filters: ${filteredVentas.length}`);
  ...
  console.log(`📊 Ventas reales (sin GR.ART.FICTICIO): ${ventasReales.length}`);
  console.log(`📊 Ventas ficticio (GR.ART.FICTICIO): ${ventasFicticio.length}`);
  
  // DEBUG: Verificar ventas con cantidad negativa (devoluciones)
  const devolucionesEnDataset = filteredVentas.filter(v => (v.cantidad || 0) < 0);
  console.log(`🔍 DEBUG - Total ventas con cantidad < 0 (devoluciones): ${devolucionesEnDataset.length}`);
  
  if (devolucionesEnDataset.length > 0) {
    console.log(`🔍 Muestra de devoluciones (primeras 3):`);
    devolucionesEnDataset.slice(0, 3).forEach((dev, idx) => {
      console.log(`   ${idx + 1}. Temporada: ${dev.temporada}, Cantidad: ${dev.cantidad}, Subtotal: ${dev.subtotal}, Familia: ${dev.descripcionFamilia}`);
    });
    
    // Contar devoluciones por temporada
    const devolucionesPorTemporada = devolucionesEnDataset.reduce((acc, dev) => {
      const temp = dev.temporada || 'SIN_TEMPORADA';
      acc[temp] = (acc[temp] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    
    console.log(`🔍 Devoluciones por temporada:`, devolucionesPorTemporada);
  } else if (filters?.temporada) {
    console.log(`⚠️  NO hay devoluciones (cantidad < 0) en la temporada filtrada: ${filters.temporada}`);
    console.log(`⚠️  Esto podría ser normal si esa temporada no tiene devoluciones registradas`);
  }
  ...
  // Calcular KPIs generales (sin ficticio) - MATCH STREAMLIT EXACTLY
  ...
  // total_neto_reales = ventas_positivas_reales (NO se resta devoluciones)
  // tasa_devolucion_reales = (devoluciones_reales / total_neto_reales) * 100
  const kpisGenerales = {
    ventasBrutas: ventasPositivasReales + devolucionesReales,  // FIXED: sum not subtract
    devoluciones: devolucionesReales,
    totalNeto: ventasPositivasReales,  // FIXED: no subtraction
  ...
  console.log(`   Ventas Positivas (Total Neto): ${ventasPositivasReales.toFixed(2)}€`);
  console.log(`   Devoluciones: ${devolucionesReales.toFixed(2)}€`);
  console.log(`   Ventas Brutas (Positivas + Devoluciones): ${kpisGenerales.ventasBrutas.toFixed(2)}€`);
  console.log(`   Tasa Devolución: ${kpisGenerales.tasaDevolucion.toFixed(1)}%`);
  ...
  // Calcular KPIs por tipo de tienda - FIXED: Incluir TODAS las ventas (positivas Y devoluciones)
  // El problema era que solo se analizaban ventas positivas, excluyendo devoluciones de tiendas físicas
  const ventasOnlinePositivas = ventasReales.filter(v => 
    (v.cantidad || 0) > 0 && v.tienda && v.tienda.toUpperCase().includes('ONLINE')
  );
  const ventasFisicasPositivas = ventasReales.filter(v => 
    (v.cantidad || 0) > 0 && v.tienda && !v.tienda.toUpperCase().includes('ONLINE')
  );
  
  // CRITICAL FIX: Construir devoluciones desde enrichedVentas (ANTES del filtro de temporada)
  // para incluir TODAS las devoluciones incluyendo las con temporada A1900
  const ventasRealesUnfiltered = enrichedVentas.filter(v => v.descripcionFamilia !== 'GR.ART.FICTICIO');
  
  const devolucionesOnline = ventasRealesUnfiltered.filter(v => 
    (v.cantidad || 0) < 0 && v.tienda && v.tienda.toUpperCase().includes('ONLINE')
  );
  const devolucionesFisicas = ventasRealesUnfiltered.filter(v => 
    (v.cantidad || 0) < 0 && v.tienda && !v.tienda.toUpperCase().includes('ONLINE')
  );
  
  console.log(`🔍 DEBUG Devoluciones por tipo de tienda (desde enrichedVentas SIN filtro temporada):`);
  console.log(`   Devoluciones Online: ${devolucionesOnline.length} registros`);
  console.log(`   Devoluciones Físicas: ${devolucionesFisicas.length} registros`);
  console.log(`   Total devoluciones: ${devolucionesOnline.length + devolucionesFisicas.length}`);
  
  const kpisTienda = {
    tiendasFisicas: new Set(ventasFisicasPositivas.map(v => v.codigoTienda).filter(Boolean)).size,
    ventasFisicas: ventasFisicasPositivas.reduce((sum, v) => sum + (v.subtotal || 0), 0),
    tiendasOnline: new Set(ventasOnlinePositivas.map(v => v.codigoTienda).filter(Boolean)).size,
    ventasOnline: ventasOnlinePositivas.reduce((sum, v) => sum + (v.subtotal || 0), 0),
  };
  
  // Ventas mensuales por tipo - INCLUIR ventas positivas Y devoluciones (negativas)
  const ventasMensualesMap = new Map<string, { fisica: { cantidad: number; beneficio: number }, online: { cantidad: number; beneficio: number } }>();
  
  // CRITICAL FIX: SOLO incluir ventas POSITIVAS de filteredVentas (evitar doble conteo)
  filteredVentas.forEach(v => {
    // SOLO ventas positivas, las devoluciones se agregan después
  ...
  // CRITICAL FIX: Agregar TODAS las devoluciones físicas (de enrichedVentas, incluyendo A1900)
  devolucionesFisicas.forEach(d => {
    if (!d.mes) return;
    
    if (!ventasMensualesMap.has(d.mes)) {
      ventasMensualesMap.set(d.mes, {
        fisica: { cantidad: 0, beneficio: 0 },
        online: { cantidad: 0, beneficio: 0 }
      });
    }
    
    const data = ventasMensualesMap.get(d.mes)!;
    data.fisica.cantidad += d.cantidad || 0; // cantidad negativa
    data.fisica.beneficio += d.subtotal || 0; // subtotal negativo
  });

  // CRITICAL FIX: Agregar TODAS las devoluciones online (de enrichedVentas, incluyendo A1900)
  devolucionesOnline.forEach(d => {
    if (!d.mes) return;
    ...
    const data = ventasMensualesMap.get(d.mes)!;
    data.online.cantidad += d.cantidad || 0; // cantidad negativa
    data.online.beneficio += d.subtotal || 0; // subtotal negativo
  });
  ...
  // Ranking de tiendas - INCLUIR ventas positivas Y devoluciones (negativas)
  ...
  devolucionesFisicas.forEach(d => {
    ...
    if (!tiendasMap.has(d.tienda)) {
      tiendasMap.set(d.tienda, { unidades: 0, beneficio: 0 });
    }
    
    const data = tiendasMap.get(d.tienda)!;
    data.unidades += d.cantidad || 0; // negativo
    data.beneficio += d.subtotal || 0; // negativo
  });
