...
// Cache for normalized column names to avoid repeated toLowerCase() calls
const columnNormCache = new Map<string, string>();
function normalizeColumn(col: string): string {
  if (!columnNormCache.has(col)) {
    columnNormCache.set(col, col.trim().toLowerCase());
  }
  return columnNormCache.get(col)!;
}

function mapRow(row: any, mapping: Record<string, string>, rowNormalizedKeys?: Map<string, string>): any {
  const mapped: any = {};
  
  // Build normalized keys once if not provided
  if (!rowNormalizedKeys) {
    rowNormalizedKeys = new Map();
    for (const key of Object.keys(row)) {
      rowNormalizedKeys.set(normalizeColumn(key), key);
    }
  }
  
  // Fast path: exact match mapping
  ...
  // Slow path: normalized matching only for unmapped fields
  for (const [excelCol, schemaCol] of Object.entries(mapping)) {
    if (mapped[schemaCol] === undefined) {
      const normalizedExcelCol = normalizeColumn(excelCol);
      const actualKey = rowNormalizedKeys.get(normalizedExcelCol);
      if (actualKey) {
...
// Cache for formatted dates to avoid repeated conversions
const dateFormatCache = new Map<any, string>();

function formatDate(excelDate: any): string {
  // Check cache first for significant performance improvement
  if (dateFormatCache.has(excelDate)) {
    return dateFormatCache.get(excelDate)!;
  }
  ...
  try {
    ...
    // Si es un objeto Date válido, convertir directamente (most common case first)
    if (excelDate instanceof Date) {
      if (isNaN(excelDate.getTime())) return '';
      result = excelDate.toISOString().split('T')[0];
      dateFormatCache.set(excelDate, result);
      return result;
    }
    
    // If it's already a string in ISO format (fast path)
    if (typeof excelDate === 'string') {
      const trimmed = excelDate.trim();
      if (!trimmed) return '';
      
      if (trimmed.match(/^\d{4}-\d{2}-\d{2}/)) {
        result = trimmed.split('T')[0].split(' ')[0];
        dateFormatCache.set(excelDate, result);
        return result;
      }
    ...
    if (typeof excelDate === 'number') {
      ...
      const date = new Date(excelEpoch.getTime() + (excelDate - 1) * 86400000); // Pre-calculated milliseconds
...
export function processExcelFile(buffer: Buffer): {
  ...
  if (ventasSheetName) {
    ...
    // Build normalized keys map once for all rows (major performance optimization)
    const rowNormalizedKeys = ventasRaw.length > 0 ? new Map<string, string>() : undefined;
    if (rowNormalizedKeys && ventasRaw.length > 0) {
      const firstRow = ventasRaw[0] as any;
    ...
    const ventasWithData = ventasRaw
      .map((row: any, index: number) => {
        try {
          const mapped = mapRow(row, effectiveMapping, rowNormalizedKeys);
          
          // CRITICAL DEBUG: Log first 3 DEVOLUCIONLINE rows to see original temporada from Excel
          if (index < 10 && mapped.cantidad < 0 && String(mapped.codigoUnico || '').includes('DEVOLUCION')) {
            console.log(`🔍 DEVOLUCION RAW #${index}:`);
            console.log(`   Excel temporada original: "${row['Temporada'] || row['temporada'] || 'NOT_FOUND'}"`);
            console.log(`   Mapped temporada: "${mapped.temporada}"`);
            console.log(`   Codigo: "${mapped.codigoUnico}"`);
            console.log(`   Cantidad: ${mapped.cantidad}`);
          }
          
          // Clean and convert numeric fields (inline for performance)
    ...
    // DEBUG: Analyze devoluciones (negative cantidad) in the processed data
    const devoluciones = ventas.filter(v => v.cantidad < 0);
    console.log(`\n🔍 ANÁLISIS DE DEVOLUCIONES (cantidad < 0):`);
    console.log(`   Total devoluciones en dataset procesado: ${devoluciones.length} de ${ventas.length} ventas`);
    
    if (devoluciones.length > 0) {
      // Show distribution by temporada
      const devolucionesPorTemporada = devoluciones.reduce((acc, dev) => {
        const temp = dev.temporada || 'SIN_TEMPORADA';
        acc[temp] = (acc[temp] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);
      console.log(`   Distribución por temporada:`, devolucionesPorTemporada);
      
      // Show first 5 devoluciones with ALL fields
      console.log(`\n   Muestra de las primeras 5 devoluciones completas:`);
      devoluciones.slice(0, 5).forEach((dev, idx) => {
        console.log(`   ${idx + 1}. Temporada="${dev.temporada}", Cantidad=${dev.cantidad}, Subtotal=${dev.subtotal}, Familia="${dev.familia}", Tienda="${dev.tienda}"`);
      });
    } else {
      console.log(`   ⚠️  NO se encontraron devoluciones (cantidad < 0) en el dataset procesado`);
    }
    console.log(`🔍 FIN ANÁLISIS DE DEVOLUCIONES\n`);
  }
