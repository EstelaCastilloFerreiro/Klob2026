...
export default function SentimentAnalysis() {
  ...
  const fetchMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest("POST", "/api/sentiment/fetch", { clientId });
    },
    onSuccess: () => {
      toast({
        title: t.app.toast.dataUpdated,
        description: t.app.toast.commentsAnalyzed,
      });
    ...
    onError: (error: any) => {
      toast({
        title: t.app.toast.error,
        description: error.message || t.app.toast.errorFetchingData,
        variant: "destructive",
      });
    },
  });
  ...
  const pieData = charts
    ? [
        { name: t.app.sentiment.positive, value: charts.sentimentDistribution.positivo, color: COLORS.positivo },
        { name: t.app.sentiment.neutral, value: charts.sentimentDistribution.neutro, color: COLORS.neutro },
        { name: t.app.sentiment.negative, value: charts.sentimentDistribution.negativo, color: COLORS.negativo },
      ]
    : [];
  ...
  const channelData = charts
    ? [
        {
          name: t.app.sentiment.instagram,
        ...
        {
          name: t.app.sentiment.googleReviews,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50/30 via-white to-yellow-50/20 dark:from-gray-900 dark:via-gray-800 dark:to-gray-950 p-6" data-testid="page-sentiment">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold bg-gradient-to-r from-yellow-600 to-stone-600 bg-clip-text text-transparent tracking-tight" data-testid="text-title">
              {t.app.sentiment.title}
            </h1>
            <p className="text-gray-600 dark:text-gray-400 mt-2" data-testid="text-subtitle">
              {t.app.sentiment.description}
            </p>
          </div>
          <Button
          ...
          >
            {fetchMutation.isPending ? (
              <>
                ...
                {t.app.toast.analyzing}
              </>
            ) : (
              <>
                <RefreshCw className="mr-2 h-4 w-4" />
                {t.app.toast.updateData}
              </>
            )}
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="p-6 bg-white dark:bg-gray-800" data-testid="card-global">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">{t.app.sentiment.globalSentiment}</p>
          ...
          <Card className="p-6 bg-white dark:bg-gray-800" data-testid="card-social">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">{t.app.sentiment.socialMedia}</p>
          ...
          <Card className="p-6 bg-white dark:bg-gray-800" data-testid="card-reviews">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">{t.app.sentiment.reviews}</p>
          ...
          <Card className="p-6 bg-white dark:bg-gray-800" data-testid="card-total">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">{t.app.sentiment.commentsAnalyzed}</p>
                <p className="text-3xl font-bold mt-2 text-yellow-700 dark:text-yellow-300" data-testid="text-total-comments">
        ...
        <div className="flex gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            <label className="text-sm text-gray-600 dark:text-gray-400">{t.app.sentiment.channel}:</label>
            <Select value={canalFilter} onValueChange={setCanalFilter}>
              <SelectTrigger className="w-[200px]" data-testid="select-canal">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="todos">{t.app.sentiment.all}</SelectItem>
                <SelectItem value="instagram">{t.app.sentiment.socialMediaOnly}</SelectItem>
                <SelectItem value="google_reviews">{t.app.sentiment.reviewsOnly}</SelectItem>
              </SelectContent>
            </Select>
          </div>

          <div className="flex items-center gap-2">
            <label className="text-sm text-gray-600 dark:text-gray-400">{t.app.sentiment.topic}:</label>
            <Select value={temaFilter} onValueChange={setTemaFilter}>
              ...
              <SelectContent>
                <SelectItem value="todos">{t.app.sentiment.all}</SelectItem>
                <SelectItem value="producto">{t.app.sentiment.product}</SelectItem>
                <SelectItem value="talla">{t.app.sentiment.size}</SelectItem>
                <SelectItem value="calidad">{t.app.sentiment.quality}</SelectItem>
                <SelectItem value="precio">{t.app.sentiment.price}</SelectItem>
                <SelectItem value="envio">{t.app.sentiment.shipping}</SelectItem>
                <SelectItem value="tienda">{t.app.sentiment.store}</SelectItem>
                <SelectItem value="web">{t.app.sentiment.website}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
        ...
        <Tabs defaultValue="charts" className="w-full">
          <TabsList className="grid w-full max-w-md grid-cols-2">
            ...
            <TabsTrigger value="comments" data-testid="tab-comments">{t.app.sentiment.comments}</TabsTrigger>
          </TabsList>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card className="p-6 bg-white dark:bg-gray-800" data-testid="card-donut">
                <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">
                  {t.app.sentiment.sentimentDistribution}
                </h3>
                {loadingCharts ? (
              ...
              <Card className="p-6 bg-white dark:bg-gray-800" data-testid="card-by-channel">
                <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">
                  {t.app.sentiment.channelComparison}
                </h3>
                {loadingCharts ? (
                  <div className="h-[300px] flex items-center justify-center">
                    <Loader2 className="h-8 w-8 animate-spin text-gray-400" />
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-xs text-gray-500 dark:text-gray-400">
                              {comment.canal === "instagram" ? t.app.sentiment.instagram : t.app.sentiment.googleReviews}
                            </span>
